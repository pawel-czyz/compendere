...
\begin{defn}
  Obiekt $P$ nazywamy \emph{początkowym} jeśli dla każdego obiektu $A$ \emph{istnieje dokładnie jeden} morfizm $P\to A$. Obiekt początkowy wygląda na diagramie jak:
        \begin{center}\begin{tikzcd} P \ar[r, "!", dashed] & A \end{tikzcd} \end{center}
\end{defn}
...
\begin{defn}
  ...
  \begin{center}\begin{tikzcd} A \ar[r, "!", dashed] & K \end{tikzcd}\end{center}
\end{defn}
...
% \begin{defn}
%   Jeśli obiekt $Z$ jest jednocześnie obiektem początkowym i końcowym, to nazywamy go obiektem \emph{zerowym}. Często jest też oznaczany przez 0 (choć niektórzy korzystają z tego oznaczenia dla obiektu początkowego).
% \end{defn}
%
% \begin{exmp}
...
%
% \begin{exmp}
%   W $\Grp$ obiektem zerowym jest grupa trywialna. Analogicznie w $\Vect k$ jest to przestrzeń trywialna.
% \end{exmp}
%
% \begin{exc}
...
\begin{defn}
  Rozważmy równoległe strzałki \begin{tikzcd} X \ar[r, "f", shift left=.75ex] \ar[r, "g", shift right=.75ex, swap] & Y \end{tikzcd}, to jest -- dwie strzałki o wspólnej dziedzinie i przeciwdziedzinie (niekoniecznie $f=g$, więc ten diagram nie jest do końca przemienny...). \emph{Ekwalizatorem} nazywamy obiekt $E$ i morfizm $\morph eEX$ taki, że:
  \begin{enumerate}
...
\begin{exc}
  Ustalmy, że pracujemy w kategorii $\Vect k$. Pokaż, że ekwalizatorem morfizmów \begin{tikzcd} X \ar[r, "f", shift left=.75ex] \ar[r, "g", shift right=.75ex, swap] & Y \end{tikzcd} jest jądro ich różnicy (to jest $\ker (f-g)$) wraz z inkluzją.
\end{exc}
...
\begin{remk}[\starred]
    ...
    W ten sposób możemy ,,kategoryjnie'' zdefiniować jądro $f$ jako ekwalizator \begin{tikzcd} X \ar[r, "f", shift left=.75ex] \ar[r, "0", shift right=.75ex, swap] & Y \end{tikzcd}.
\end{remk}
